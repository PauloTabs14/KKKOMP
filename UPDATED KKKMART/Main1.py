...
class LoginUI(QWidget):
    ...
    def login_validation(self):
        try:
            ...
            # Create the home page with user type information
            try:
...
class HomePageUI(QMainWindow):
    ...
    def homepage_setup_ui(self):
        ...
        profile_btn.setIcon(QIcon("profile_icon.png"))
    ...
    def setup_navigation_bar(self, main_layout):
        ...
        nav_items = [
            ...
            ("inventory", "Inventory", self.open_inventory),
...
class OrderHistoryUI(QWidget):
    ...
    def build_ui(self):
        ...
        if not orders:
            empty = QLabel("No previous orders found.")
            empty.setAlignment(Qt.AlignCenter)
            orders_layout.addStretch()
            orders_layout.addWidget(empty)
            orders_layout.addStretch()
        else:
            for idx, order in enumerate(reversed(orders), 1):  # latest first
                frame = QFrame()
                frame.setStyleSheet("border: 1px solid #ccc; border-radius: 8px; padding: 8px;")
                frm_layout = QVBoxLayout()
                frm_layout.setSpacing(4)

                dt_lbl = QLabel(f"Order #{idx} • {order['datetime']}")
                dt_lbl.setStyleSheet("font-weight: bold; font-size: 13px;")

                items = order.get('items', [])
                items_text = ", ".join([i.get('name', '') for i in items]) if items else "(no items)"
                items_lbl = QLabel(f"Items: {items_text}")

                subtotal_lbl = QLabel(f"Subtotal: ${order.get('subtotal')}")
                discount_lbl = QLabel(f"Discount: ${order.get('discount')}")
                total_lbl = QLabel(f"Total Paid: ${order.get('total')}")
                total_lbl.setStyleSheet("color: red; font-weight: bold;")

                method_lbl = QLabel(f"Payment Method: {order.get('payment_method')}")

                for w in (dt_lbl, items_lbl, subtotal_lbl, discount_lbl, total_lbl, method_lbl):
                    frm_layout.addWidget(w)
                frame.setLayout(frm_layout)
                orders_layout.addWidget(frame)

        orders_layout.addStretch()
        container.setLayout(orders_layout)
        ...
        orders = self.load_orders()
        if not orders:
            empty_label = QLabel("No previous orders found.")
            empty_label.setAlignment(Qt.AlignCenter)
            layout.addStretch()
            layout.addWidget(empty_label)
            layout.addStretch()
        else:
            for idx, order in enumerate(reversed(orders), 1):  # show latest first
                order_frame = QFrame()
                order_frame.setFrameShape(QFrame.StyledPanel)
                order_layout = QVBoxLayout()

                dt_label = QLabel(f"Order #{idx} - {order['datetime']}")
                dt_label.setStyleSheet("font-weight: bold; font-size: 13px;")
                items_text = ", ".join([itm.get('name', '') for itm in order.get('items', [])])
                items_label = QLabel(f"Items: {items_text}")
                total_label = QLabel(f"Total Paid: ${order['total']}")
                total_label.setStyleSheet("color: red;")

                for w in (dt_label, items_label, total_label):
                    order_layout.addWidget(w)
...
class ProfileUI(QWidget):
    ...
    def build_ui(self):
        ...
        avatar = QLabel(); avatar.setPixmap(QPixmap("UserIcon.png").scaled(80,80,Qt.KeepAspectRatio)); avatar.setAlignment(Qt.AlignCenter)
        layout.addWidget(avatar)
        name_lbl = QLabel(f"Username: {username}"); email_lbl=QLabel(f"Email: {email}"); phone_lbl=QLabel(f"Phone: {phone}")
        for lbl in (name_lbl,email_lbl,phone_lbl):
            lbl.setFont(QFont("Arial",12)); lbl.setAlignment(Qt.AlignCenter)
            layout.addWidget(lbl)
        edit_btn = QPushButton("Edit Profile"); edit_btn.setStyleSheet("background:red;color:white;border-radius:10px;height:35px;"); edit_btn.clicked.connect(self.open_edit)
        back_btn = QPushButton("<- Back"); back_btn.clicked.connect(self.go_back)
        layout.addWidget(edit_btn); layout.addWidget(back_btn)
        self.setLayout(layout)

    def open_edit(self):
        self.edit_window = AccountSecurityUI(self.homepage_window, self.username)
        self.edit_window.show(); self.close()

    def go_back(self):
        self.homepage_window.show(); self.close()
